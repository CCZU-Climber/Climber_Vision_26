...
const std::string keys =
  ...
  "{signal-mode m  |     triangle_wave   | Signal mode: triangle_wave / step / circle}"
  "{axis        x  |         yaw         | Controlled axis: yaw / pitch}"
  "{@config-path   | configs/uav.yaml    | Positional yaml config path }";
...
int main(int argc, char * argv[])
{
  ...
  // 发送初始指令使能控制
  io::Command init_cmd{0};
  init_cmd.control = 1;
  cboard.send(init_cmd);
  ...
  while (!exiter.exit()) {
    ...
    Eigen::Vector3d eulers = tools::eulers(q, 2, 1, 0);  // yaw, pitch, roll
    ...
    if (signal_mode == "triangle_wave") {
      ...
      command.shoot = 0;
      if (axis_index == 0) {
        command.yaw = tools::limit_rad(cmd_angle / 57.3);
        command.pitch = 0;
      } else {
        command.yaw = 0;
        command.pitch = tools::limit_rad(cmd_angle / 57.3);
      }
